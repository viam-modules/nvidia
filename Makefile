...
module: build
	...
	tar czf $(BIN_OUTPUT_PATH)/module.tar.gz $(BIN_OUTPUT_PATH)/nvidia meta.json

build: build-go

build-go:
	rm -f $(BIN_OUTPUT_PATH)/nvidia
	go build -tags no_cgo,osusergo,netgo -ldflags="-extldflags=-static $(COMMON_LDFLAGS)"  -o $(BIN_OUTPUT_PATH)/nvidia main.go
